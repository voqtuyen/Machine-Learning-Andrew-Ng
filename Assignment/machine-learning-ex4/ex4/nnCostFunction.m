...
y_coded = recodeY(num_labels,y);
J = -1/m * sum(sum(y_coded .* log(a2) + (1 - y_coded) .* log(1 - a2))) + lambda/(2*m)*(sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));
